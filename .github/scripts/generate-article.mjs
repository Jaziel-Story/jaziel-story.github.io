...
async function main() {
  ...
  if (!categoryHint || !categoryHint.trim()) throw new Error("Category Hint is required for AI Writer generation.");
...
- Use the Category Hint exactly as provided for the category field. Do not rename, translate, or invent a different category.
...
Respond with ONLY a single JSON object (no markdown fences, no comments, no text outside the JSON) with exactly these keys:
{
  ...
  "category": string (exactly the provided Category Hint),
...
const RESPONSE_SCHEMA = {
  ...
  required: ["title", "description", "category", "dek", "intro", "sections", "closing", "tags", "seoTitle", "seoDescription"]
};
  ...
  for (let attempt = 1; attempt <= GEMINI_MAX_ATTEMPTS; attempt++) {
    try { return await callGemini(prompt); }
      ...
      if (!transient || isLastAttempt) {
        if (transient && isLastAttempt && attempt > 1) err.message = `${err.message} (gave up after ${attempt} attempts)`;
        throw err;
      }
...
async function callGemini(prompt) {
  ...
  try {
    ...
    if (err.name === "AbortError") { const e = new Error(`Gemini request timed out after ${GEMINI_TIMEOUT_MS / 1000}s.`); e.transient = true; throw e; }
    const e = new Error(`Could not reach the Gemini API: ${err.message}`); e.transient = true; throw e;
  } finally { clearTimeout(timeout); }
  if (!res.ok) {
    const detail = await safeText(res);
    const message = describeGeminiHttpError(res.status, detail);
    const e = new Error(message); e.transient = res.status === 429 || res.status >= 500; throw e;
  }
...
function parseJsonLoosely(text) {
  ...
  try { return JSON.parse(cleaned); }
    ...
    if (first !== -1 && last !== -1 && last > first) { try { return JSON.parse(cleaned.slice(first, last + 1)); } catch {} }
...
function validateDraft(draft) {
  ...
  if (!Array.isArray(draft?.sections) || draft.sections.length === 0) errors.push('"sections" must be a non-empty array');
  else draft.sections.forEach((s, i) => {
    ...
    if (!Array.isArray(s.paragraphs) || s.paragraphs.length === 0 || !s.paragraphs.every(p => typeof p === "string" && p.trim())) errors.push(`sections[${i}].paragraphs must be a non-empty array of non-empty strings`);
  });
  if (!Array.isArray(draft?.tags) || !draft.tags.every(t => typeof t === "string")) errors.push('"tags" must be an array of strings');
  if (draft?.bibleVerse !== undefined) {
    ...
    if (looksEmpty) delete draft.bibleVerse;
  }
